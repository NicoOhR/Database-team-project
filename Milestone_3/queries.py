...
Milestone 3 note:
This module stays as the legacy/console read-only layer.
The GUI-specific read-only layer lives in ``milestone3_queries.py``.
...
def search_direct_trips(origin: str, destination: str, date_str: str | None = None) -> list[dict[str, Any]]:
    """Return direct flight legs between two airports, optionally for one date."""
    if date_str is None:
        return _fetch_all(
            """
            SELECT
    ...
    return _fetch_all(
        ...
        SELECT
            ...
            fl.Dep_airport_code AS Origin,
            fl.Arr_airport_code AS Destination,
            li.Dep_time AS Scheduled_dep_time,
            li.Arr_time AS Scheduled_arr_time
...
def search_one_connection_trips(origin: str, destination: str, date_str: str | None = None) -> list[dict[str, Any]]:
    """Return trips that use exactly one connecting airport."""
    date_clause = ""
    params: list[Any] = [origin, destination]
    if date_str is not None:
        date_clause = "AND li1.Date = %s"
        params.append(date_str)

    return _fetch_all(
        f"""
        SELECT
            li1.Flight_number AS First_flight,
            first_flight.Airline AS First_airline,
            li1.Leg_no AS First_leg,
            first_leg.Dep_airport_code AS Origin,
            first_leg.Arr_airport_code AS Connection,
            li1.Dep_time AS First_dep_time,
            li1.Arr_time AS First_arr_time,
            li2.Flight_number AS Second_flight,
            second_flight.Airline AS Second_airline,
            li2.Leg_no AS Second_leg,
            second_leg.Arr_airport_code AS Destination,
            li2.Dep_time AS Second_dep_time,
            li2.Arr_time AS Second_arr_time
        FROM LEG_INSTANCE li1
        JOIN FLIGHT_LEG first_leg
          ON first_leg.Flight_number = li1.Flight_number
         AND first_leg.Leg_no = li1.Leg_no
        JOIN FLIGHT first_flight ON first_flight.Number = li1.Flight_number
        JOIN LEG_INSTANCE li2
          ON li2.Date IN (li1.Date, DATE_ADD(li1.Date, INTERVAL 1 DAY))
        JOIN FLIGHT_LEG second_leg
          ON second_leg.Flight_number = li2.Flight_number
         AND second_leg.Leg_no = li2.Leg_no
         AND second_leg.Dep_airport_code = first_leg.Arr_airport_code
        JOIN FLIGHT second_flight ON second_flight.Number = li2.Flight_number
        WHERE first_leg.Dep_airport_code = %s
          ...
          AND (
                ...
                TIMESTAMP(li1.Date, li1.Arr_time) + INTERVAL 1 HOUR
              )
...
def search_trips_for_airports(
    ...
    for origin in origin_airports:
        for destination in destination_airports:
            ...
            for trip in search_direct_trips(origin_code, destination_code, date_str):
                key = (trip["Flight_number"], trip["Leg_no"])
                if key not in seen_direct:
                    seen_direct.add(key)
                    direct.append(trip)

            for trip in search_one_connection_trips(origin_code, destination_code, date_str):
                key = (
                    trip["First_flight"],
                    trip["First_leg"],
                    trip["Second_flight"],
...
def safe_search_trips(
    origin: str, destination: str, date_str: str | None = None
    ...
    try:
        ...
        result = search_trips_for_airports(origin_airports, destination_airports, date_str)
